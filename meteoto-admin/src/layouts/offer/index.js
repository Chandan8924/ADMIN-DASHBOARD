...
import BillingInformation from "layouts/offer/data";
...
export default function Billing() {
  ...
  const getAllOffer = () => {
    try {
      setSkelLoading(true);
      // setLoading(true);
      fetch(`${process.env.REACT_APP_API}/api/getAllOffers`, {
        method: "GET",
      })
        .then((res) => res.json())
        .then((result) => {
                ...
                <Grid item xs={12} lg={10}>
                  <Grid container spacing={4}>
                    <Grid item xs={12} md={6} xl={3}>
                      <MiniStatisticsCard
                        title={{ text: "Total Offer" }}
                        count={count.totalOffer} //"53,000"
