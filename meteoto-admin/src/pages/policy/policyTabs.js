...
export function BasicTabs(props) {
      ...
      .then((resp) => {
        ...
        if (resp.success) {
          ...
          editorState.push(
            EditorState.createWithContent(
              ContentState.createFromBlockArray(
                convertFromHTML(resp?.data?.return_policy)
              )
            )
          );
  ...
  const submitPolicy = (ev) => {
    fetch(`${process.env.REACT_APP_API}/api/update/company`, {
      method: "PUT",
      headers: {
    ...
    <Box sx={{ width: "100%" }}>
      <Box sx={{ borderBottom: 1, borderColor: "divider" }}>
        ...
        >
          ...
          <Tab label="Return & Refund Policy" onClick={() => setValue(2)} />;
        </Tabs>
      </Box>
      {loading ? (
        <Loading />
              ...
              >
                ...
                >
                  <SoftButton
                    ...
                    color="info"
                    type="submit"
                    onClick={() => submitPolicy("return_policy")}
                  >
                    Submit
                  </SoftButton>
                </SoftBox>
