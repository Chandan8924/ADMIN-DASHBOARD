...
import CreateExpense from "../../layouts/tables/form";
...
function Tables() {
  ...
  const deleteExpense = (ev) => {
    try {
      fetch(`${process.env.REACT_APP_API}/api/v1/delete/expense/${ev}`, {
        method: "DELETE",
        headers: {
          Authorization: token,
        },
  ...
  const getCustomerList = () => {
    try {
      setLoading(true);
      fetch(`${process.env.REACT_APP_API}/api/v1/get/all/expense`, {
        method: "GET",
        headers: {
          Authorization: token,
        },
  ...
  return (
    <DashboardLayout>
      ...
      <SoftBox py={3}>
        <SoftBox mb={3}>
          <Card>
            <SoftButton size="small" color="black" fullWidth onClick={() => handleShowForm(null)}>
              Click here to Create Expense
            </SoftButton>
            <SoftBox
              sx={{
